...
- **同一终点 H 对比**：所有方案一律模拟到同一 H = max(贷款还清月, 退休年偏移+12（含退休当年全年）, 自定义终点年偏移+12, 最晚事件月, 有界重复事件末次发生月)，先还清的靠理财滚存——这是「真实节省 = 方案期末净资产 − 基准期末净资产」可比的前提；无界重复与定投不延伸终点，随 H 截断
...
### 宽心指数（0–100）
coverage(m) = liquid(m)/monthlyOutgo(m)，liquid=cash+Σpools（公积金不计）。分段映射：≥36月→80~100，12–36→50~80，6–12→25~50（焦虑区），<6→0~25（危险红）。综合分 = 0.20·当前跑道 + 0.35·最危险时刻(base) + 0.30·压力跑道(stress) + 0.15·断裂惩罚(100−brokeMonths×15)；硬帽：base 断裂 ≤24，仅 stress 断裂 ≤39。
...
2. 手工对账案例：100万商贷30年4.2%等额本息，月供应为 ≈4890.17 元（M = P·i·(1+i)^n/((1+i)^n−1)，i=年利率/12，与银行计算器/Excel PMT 一致）；提前还款10万缩期限后的新月供/剩余期数与主流工具交叉核对
